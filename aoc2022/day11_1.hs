...
parseInput = map (parseMonkey . tail . lines) . splitOn "\n\n"
  where
    parseMonkey [its, op, tst, t, f] = Monkey (parseItems its) (parseOp op) (readLast tst) (readLast t, readLast f) 0
...
updateMonkey monkeys (i, item) = take i monkeys ++ [nmonkey] ++ drop (i + 1) monkeys
  where
    Monkey mitems mop mtest mthrow minspection = monkeys !! i
...
monkeyTurn rel monkeys i = take i nmonkeys ++ [nmonkey] ++ drop (i + 1) nmonkeys
  where
    Monkey mitems mop mtest (t, f) minspection = monkeys !! i
...
main = do
  input <- parseInput <$> readFile "input/day11_1.in"
